...
To match all of the thousands (from 1000 to 9000) except for "5000" and "6000", we _could_ use the regex `[1234789]000` or `[1-47-9]000`, but there's a more efficient solution: the caret `^`.

When used at the start of a character set (immediately after the opening square bracket), a caret (^) signifies that we want to match anything _except_ for what's in the character set, so we could do the following to get the same result as above:
